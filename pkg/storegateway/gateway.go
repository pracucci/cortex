...
func newStoreGateway(gatewayCfg Config, storageCfg cortex_tsdb.Config, bucketClient objstore.Bucket, ringStore kv.Client, logLevel logging.Level, logger log.Logger, reg prometheus.Registerer) (*StoreGateway, error) {
	...
	if gatewayCfg.ShardingEnabled {
		lifecyclerCfg, err := gatewayCfg.ShardingRing.ToBasicLifecyclerConfig()
		if err != nil {
			return nil, errors.Wrap(err, "invalid ring lifecycler config")
		}

		g.ringLifecycler, err = ring.NewBasicLifecycler(lifecyclerCfg, RingName, RingKey, ringStore, g, logger, reg)
		if err != nil {
			return nil, errors.Wrap(err, "create ring lifecycler")
		}

		ringCfg := gatewayCfg.ShardingRing.ToRingConfig()
		g.ring, err = ring.NewWithStoreClient(ringCfg, RingName, RingKey, ringStore)
		if err != nil {
		...
		// sharding is enabled.
		filters = append(filters, NewRingShardedMetaFilter(g.ring, g.ringLifecycler.GetInstanceID()))
	}

	g.stores, err = querier.NewBucketStores(storageCfg, filters, bucketClient, logLevel, logger, reg)
	if err != nil {
...
func (g *StoreGateway) starting(ctx context.Context) (err error) {
	...
	if g.gatewayCfg.ShardingEnabled {
		...
		if err := g.ring.WaitInstanceState(ctx, g.ringLifecycler.GetInstanceID(), ring.JOINING); err != nil {
			return err
		}
...
func (g *StoreGateway) running(ctx context.Context) error {
	...
	var ringLastTokens ring.RingTokens

	syncTicker := time.NewTicker(g.storageCfg.BucketStore.SyncInterval)
	defer syncTicker.Stop()

	if g.gatewayCfg.ShardingEnabled {
		ringLastTokens = g.ring.GetOwnedTokens(ring.BlocksSync)
		// TODO allow to configure but do not expose the config (useful in tests)
		ringTicker := time.NewTicker(time.Second)
	...
	for {
		...
		case <-ringTickerChan:
			if g.gatewayCfg.ShardingEnabled {
				// TODO test me
				currTokens := g.ring.GetOwnedTokens(ring.BlocksSync)
...
func (g *StoreGateway) OnRingInstanceRegister(_ *ring.BasicLifecycler, ringDesc ring.Desc, _ bool, instanceID string, _ ring.IngesterDesc) (ring.IngesterState, ring.Tokens) {
	// When we initialize the store-gateway instance in the ring we want to start from
	// a clean situation, so whatever is the state we set it JOINING, while we keep existing
	// tokens (if any).
	instanceTokens, takenTokens := ringDesc.TokensFor(instanceID)
	newTokens := ring.GenerateTokens(RingNumTokens-len(instanceTokens), takenTokens)
	instanceTokens = append(instanceTokens, newTokens...)

	return ring.JOINING, instanceTokens
}
...
func createBucketClient(cfg cortex_tsdb.Config, logger log.Logger, reg prometheus.Registerer) (objstore.Bucket, error) {
	...
	if reg != nil {
		bucketClient = objstore.BucketWithMetrics( /* bucket label value */ "", bucketClient, prometheus.WrapRegistererWithPrefix("cortex_store_gateway_", reg))
	}
