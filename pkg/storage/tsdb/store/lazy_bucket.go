...
func NewBucketStore(
	...
	s := &BucketStore{
		...
		queryGate: store.NewGate(
			maxConcurrent,
			extprom.WrapRegistererWithPrefix("thanos_bucket_store_series_", reg),
		),
		samplesLimiter:           store.NewLimiter(maxSampleCount, metrics.queriesDropped),
...
func (r *bucketChunkReader) preload(samplesLimiter *store.Limiter) error {
	var g run.Group
