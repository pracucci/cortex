...
var (
	...
	errDeletionMarkCorrupted = errors.New("block deletion mark corrupted")
)
...
func NewWriter(bkt objstore.InstrumentedBucket, userID string, logger log.Logger) *Writer {
	return &Writer{
		...
		userID: userID,
		logger: logger, // TODO use util.WithUserID(userID, logger)
	}
}
...
func (w *Writer) GenerateIndex(ctx context.Context, old *Index) (*Index, error) {
	...
	blocks, err := w.generateIndexBlocks(ctx, oldBlocks)
	if err != nil {
		return nil, err
	}

	blockDeletionMarks, err := w.generateIndexBlockDeletionMarks(ctx, oldBlockDeletionMarks)
	if err != nil {
		return nil, err
	}
...
func (w *Writer) generateIndexBlocks(ctx context.Context, old []*Block) ([]*Block, error) {
	...
	for id := range discovered {
		b, err := w.generateIndexBlock(ctx, id)
		if errors.Is(err, errBlockMetaNotFound) {
			level.Warn(w.logger).Log("msg", "skipped partial block when generating bucket index", "block", id.String(), "user", w.userID)
			continue
		} else if errors.Is(err, errBlockMetaCorrupted) {
			// TODO need a metric on this
			level.Error(w.logger).Log("msg", "skipped block with corrupted meta.json when generating bucket index", "block", id.String(), "user", w.userID)
			continue
		} else if err != nil {
			return nil, err
		}
...
func (w *Writer) generateIndexBlock(ctx context.Context, id ulid.ULID) (*Block, error) {
	...
	if w.bkt.IsObjNotFoundErr(err) {
		return nil, errors.Wrapf(errBlockMetaNotFound, "%v", err)
	}
	if err != nil {
		return nil, errors.Wrapf(err, "get meta file: %v", metaFile)
	}
	defer runutil.CloseWithLogOnErr(w.logger, r, "close block meta get")
	...
	if err := json.Unmarshal(metaContent, &m); err != nil {
		return nil, errors.Wrapf(errBlockMetaCorrupted, "meta.json %v unmarshal: %v", metaFile, err)
	}
...
func (w *Writer) generateIndexBlockDeletionMarks(ctx context.Context, old []*BlockDeletionMark) ([]*BlockDeletionMark, error) {
	...
	for id := range discovered {
		m, err := generateIndexBlockDeletionMark(ctx, w.bkt, id, w.logger)
		if errors.Is(err, errDeletionMarkCorrupted) {
			// TODO need a metric on this
			level.Error(w.logger).Log("msg", "skipped corrupted deletion mark when generating bucket index", "block", id.String(), "user", w.userID)
			continue
		} else if err != nil {
			return nil, err
		}
...
func generateIndexBlockDeletionMark(ctx context.Context, bkt objstore.InstrumentedBucket, id ulid.ULID, logger log.Logger) (*BlockDeletionMark, error) {
	markFile := path.Join(id.String(), metadata.DeletionMarkFilename)

	// Get the block's deletion mark file.
	r, err := bkt.ReaderWithExpectedErrs(bkt.IsObjNotFoundErr).Get(ctx, markFile)
	if err != nil {
		return nil, errors.Wrapf(err, "get deletion mark: %v", markFile)
	}
	defer runutil.CloseWithLogOnErr(logger, r, "close deletion mark get")

	markContent, err := ioutil.ReadAll(r)
	if err != nil {
		return nil, errors.Wrapf(err, "read deletion mark: %v", markFile)
	}

	// Unmarshal it.
	m := metadata.DeletionMark{}
	if err := json.Unmarshal(markContent, &m); err != nil {
		return nil, errors.Wrapf(errDeletionMarkCorrupted, "deletion-mark.json %v unmarshal: %v", markFile, err)
	}

	if m.Version != metadata.DeletionMarkVersion1 {
		return nil, errors.Errorf("unexpected deletion mark file: %s version: %d", markFile, m.Version)
	}
