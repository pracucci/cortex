...
type MarkersIndexBucket struct {
	objstore.Bucket
}

// BucketWithMarkersIndex wraps the input bucket into a bucket which keeps track of markers
// in the global markers index location too.
func BucketWithMarkersIndex(b objstore.Bucket) *MarkersIndexBucket {
	return &MarkersIndexBucket{
		Bucket: b,
	}
}

// Upload implements objstore.Bucket.
func (b *MarkersIndexBucket) Upload(ctx context.Context, name string, r io.Reader) error {
...
func (b *MarkersIndexBucket) Delete(ctx context.Context, name string) error {
	// Call the parent.
...
func (b *MarkersIndexBucket) isBlockDeletionMark(name string) (ulid.ULID, bool) {
